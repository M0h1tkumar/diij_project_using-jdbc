...
```text
2341011177_MohitKumar
```

Java root package:

```text
mohitkumar_2341011177
```
...
```bash
cd "/home/iter/Downloads/DIIJProject/DIIJ Project/DIIJ Project/2341011177_MohitKumar"
```

Create the output folder and compile:
...
```powershell
cd "C:\Users\Mohit\Downloads\DIIJ Project\2341011177_MohitKumar"
```
...
The `run.sh` script auto-compiles if needed and launches the application:
...
After compiling, run the console application.
...
```powershell
java -cp "out;lib/*" mohitkumar_2341011177.MainApp --self-test
```
...
```bash
rm -rf out libraryDB derby.log sources.txt
...
Remove-Item -Force derby.log, sources.txt -ErrorAction SilentlyContinue
